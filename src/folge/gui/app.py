...
class PipelineWizard:
    ...
    def build(self):
        ...
        with ui.column().classes('w-full h-screen p-4 gap-2'):
            with ui.row().classes('w-full items-center'):
                ...
                ui.space()
                ui.button('Exit', icon='power_settings_new',
                          on_click=self._on_exit).props('color=negative flat')
            ...
            with ui.row().classes('w-full gap-1 justify-center'):
                for i, step in enumerate(STEPS):
                    btn = ui.button(step['label'], icon=step['icon']).props(
            ...
            with ui.row().classes('w-full flex-1 gap-4'):
                with ui.column().classes('flex-1 gap-2 overflow-auto'):
                    ...
                    with ui.card().classes('w-full'):
                        ...
                        self.log = ui.log(max_lines=200).classes(
                            'w-full font-mono text-sm').style(
                    ...
                    self.progress_label = ui.label('Ready').classes('text-center text-subtext')

                    with ui.row().classes('w-full justify-center gap-4'):
                        self.run_btn = ui.button(
                            'Run Next Step', icon='play_arrow', on_click=self._on_run
                        ).props('color=primary size=lg')
                        self.retry_btn = ui.button(
                            'Retry Step', icon='refresh', on_click=self._on_retry
                        ).props('color=warning size=lg').classes('hidden')
                        self.continue_btn = ui.button(
                            'Continue to Next Step', icon='arrow_forward', on_click=self._on_continue
                        ).props('color=positive size=lg').classes('hidden')

                    with ui.row().classes('w-full justify-between'):
                        self.status_label = ui.label('Idle').classes('text-subtext')
                        ui.label('Folge Vision Pipeline').classes('text-subtext')

                self.right_panel = ui.card().classes('flex-1 gap-2 overflow-auto').style(
                    'min-height: 400px;')
                with self.right_panel:
                    ...
                    with self.codemirror_container:
                        ui.label('Run the Manual Review step to load the editor.').classes(
                            'text-subtext')
                    ui.separator()
        ...
        with ui.card().classes('w-full'):
            ...
            with ui.column().classes('w-full gap-2'):
                with ui.row().classes('w-full items-center gap-2'):
                    ...
                    ui.button('Browse', on_click=lambda: self._pick_file(
                ...
                with ui.row().classes('w-full items-center gap-2'):
                    ...
                    ui.button('Browse', on_click=lambda: self._pick_folder(
                        self.images_input
                    )).props('flat color=info')
    ...
    def _open_browser(self, target_input, mode='file', pattern='*.json'):
        ...
        def navigate_to(directory: Path, container):
            ...
            with container:
                if directory != directory.parent:
                    with ui.row().classes('w-full items-center gap-2 cursor-pointer hover:bg-surface0 p-2 rounded'):
                        ui.icon('arrow_upward', color='info')
                        up_label = ui.label(str(directory.parent)).classes('text-info flex-grow')
                        up_label.on('click', lambda: navigate_to(directory.parent, container))
                ...
                for entry in entries:
                    ...
                    with ui.row().classes('w-full items-center gap-2 cursor-pointer hover:bg-surface0 p-2 rounded'):
                        ...
                        if entry.is_dir():
                            lbl.on('click', lambda e, p=entry: navigate_to(p, container))
                        else:
        ...
        with dialog, ui.card().classes('w-full max-w-2xl'):
            ...
            ui.separator()
            file_list = ui.column().classes('w-full overflow-auto').style('max-height: 400px;')
            ui.separator()
    ...
    def _run_merge(self):
        ...
        enriched = run(guide, vision, output / 'guide.enriched.json',
        ...
        with self.codemirror_container:
            ...
            with ui.row().classes('w-full justify-center gap-4 mt-2'):
                ui.button('Save Changes', icon='save',
                          on_click=self._save_from_codemirror).props('color=info')
                ui.button('Continue to Validation', icon='arrow_forward',
                          on_click=self._on_continue).props('color=positive size=lg')
    ...
    def _run_validate(self):
        ...
        ok, issues = run(self.enriched_path, min_confidence=confidence,
                         output_dir=output, on_progress=self._on_progress)
    ...
    def _run_publish(self):
        ...
        published = run(self.enriched_path, targets, output,
                        on_progress=self._on_progress)
        ...
        try:
            ...
            if ok:
                self._schedule(lambda: (
                    ...
                    self._set_status(f'Step {step_index + 1}/7 — Waiting for approval'),
                ))
            else:
                self._schedule(lambda: (
                    self._log(f'Step {step_index} failed'),
                    self._show_buttons(run=False, retry=True, continue_=False),
                    self._set_status(f'Step {step_index + 1}/7 — Failed (click Retry)'),
                ))
        except Exception as ex:
            error_msg = str(ex)
            self._schedule(lambda msg=error_msg: (
    ...
    def _on_run(self):
        ...
        self._log(f'--- Starting Step {self.current_step + 1}: {STEPS[self.current_step]["label"]} ---')
        self._update_step_indicators()
    ...
    def _on_retry(self):
        ...
        self._log(f'--- Retrying Step {self.current_step + 1}: {STEPS[self.current_step]["label"]} ---')
