...
def check_pdfinfo_installed():
    ...
    -------
    bool
        True if pdfinfo is available on the system.
    """
    try:
        subprocess.run(
...
def check_pymupdf_installed():
    ...
    -------
    bool
        True if the fitz module can be imported.
    """
    try:
...
def validate_with_pdfinfo(pdf_path):
    ...
    tuple[list[str], list[str]]
        Tuple of (issues, successes) string lists.
    """
    try:
...
def main():
    """Run CLI validation for PDF accessibility."""
    if len(sys.argv) < 2:
        print("Usage: folge-cli validate-pdf <pdf-file>")
        print("Example: folge-cli validate-pdf output/guide.pdf")
        sys.exit(1)
