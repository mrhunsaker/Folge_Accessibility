...
#
# Folge Accessibility Pipeline -- Podman image
#
# This is the Podman counterpart of ./Dockerfile. The build instructions
# below are intentionally identical: Containerfile and Dockerfile use the
# same OCI build syntax, and Podman builds either one natively (it just
# looks for "Containerfile" before "Dockerfile" by convention). Keeping
# both files means `docker build .` and `podman build .` each pick up
# their own file name with no extra flags, while staying in sync -- if
# you change one, mirror the change in the other. All Docker-vs-Podman
# differences (host networking name, rootless UID mapping, SELinux
# volume labels) live in the compose files, not in the image build.
#
# Build:  podman build -t folge-cli -f Containerfile .
# Run:    podman run --rm folge-cli --help
#
# ------------------------------------------------------------------------
